...
function wk_album_upload(){
    $showerror = true;
    if(helper_access::check_module('album')) {
        //require_once libfile('function/spacecp'); //不知为什么这句总是不成功干脆注释。
        if($_FILES["Filedata"]['error']) {
            $file = lang('spacecp', 'file_is_too_big');
        } else {
            require_once libfile('function/home');
            $_FILES["Filedata"]['name'] = addslashes(diconv(urldecode($_FILES["Filedata"]['name']), 'UTF-8'));
            $file = wk_pic_save($_FILES["Filedata"], 0, '', true, 0);
...
function wk_pic_save($FILE, $albumid, $title, $iswatermark = true, $catid = 0) {
    global $_G, $space;
    ...
    if ($iswatermark) {
        $image->Watermark($image->target, '', 'album');
